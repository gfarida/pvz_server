...
Перед запуском убедитесь, что локальный PostgreSQL не запущен, чтобы избежать конфликта портов:
...
1. Перед запуском убедиться, что PostgreSQL локально запущен:

```bash
sudo systemctl start postgresql
```

2. Создать базу даннных:

```bash
createdb -U postgres pvz
```

3. Применить миграции для создания необходимых таблиц:
```bash
make migrate-up
```
...
| `make run`              | Запускает приложение с использованием `.env.local`                       |
...
![ER Diagram](./images/pvz.png)

У сущности «Пункт приёма заказов (pvz)» есть:

- Уникальный идентификатор
- Дата регистрации в системе
- Город

У сущности «Приёмка товара (reception)» есть:
